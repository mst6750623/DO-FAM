...
def extract_on_paths(file_paths):
    ...
    print('\t{} is starting to extract on {} images'.format(
        pid, len(file_paths)))
...
def parse_args():
    ...
    parser.add_argument('--gt_path',
                        type=str,
                        default='gt_images',
                        help='path to gt images')
    parser.add_argument('--step',
                        action='store_true',
                        help='whether to calculate each step')
...
def run(args):
    if args.step:
        for step in tqdm(sorted(os.listdir(args.output_path))):
            if not step.isdigit():
                continue
...
def run_on_step_output(step, args):
    ...
    for f in os.listdir(step_outputs_path):
        ...
        gt_path = os.path.join(args.gt_path, fname + '.jpg')
    ...
    result_str = 'New Average score is {:.4f}+-{:.4f}'.format(mean, std)
    print(result_str)

    out_path = os.path.join(os.path.dirname(args.output_path),
                            'inference_metrics')

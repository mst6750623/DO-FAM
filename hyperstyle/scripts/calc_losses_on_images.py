...
def parse_args():
	...
	parser.add_argument('--metrics', type=str, default='lpips,l2,msssim')
	parser.add_argument('--output_path', type=str, default='results')
	parser.add_argument('--gt_path', type=str, default='gt_images')
	parser.add_argument('--workers', type=int, default=1)
	parser.add_argument('--batch_size', type=int, default=1)
	parser.add_argument('--step', action='store_true', help='whether to calculate each step')
	...
	for metric in args.metrics.split(','):
		...
		if args.step:
			for step in sorted(os.listdir(args.output_path)):
				...
				print(step_outputs_path)
				if os.path.isdir(step_outputs_path):
					print('#' * 80)
					print(f'Computing {args.metric} on step: {step}')
					print('#' * 80)
			...
			if os.path.isdir(args.output_path):
				print('#' * 80)
				print(f'Computing {args.metric}')
				print('#' * 80)
				run_on_step_output(step=0, args=args)
...
def run_on_step_output(step, args):
	...
	if args.step:
		...
		step_outputs_path = args.output_path
	...
	if args.metric == 'lpips':
		...
		raise Exception(f'Not a valid metric: {args.metric}!')
	...
	result_str = 'Average loss is {:.5f}+-{:.5f}'.format(mean, std)
	...
	with open(os.path.join(out_path, f'stat_{args.metric}.txt'), 'w') as f:
		f.write(result_str)
	with open(os.path.join(out_path, f'scores_{args.metric}.json'), 'w') as f:

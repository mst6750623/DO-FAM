...
from . import restyle_e4e_encoders
...
class e4e(nn.Module):
    ...
    def load_weights(self):
        if self.opts.checkpoint_path is not None:
            print(
                f'Loading ReStyle e4e from checkpoint: {self.opts.checkpoint_path}'
            )
            ckpt = torch.load(self.opts.checkpoint_path, map_location='cpu')
            self.encoder.load_state_dict(self.__get_keys(ckpt, 'encoder'),
                                         strict=True)
        ...
        if "ffhq" in self.opts.dataset_type:
            ...
            if self.opts.input_nc != 3:
                ...
                altered_input_layer = torch.randn(shape[0],
                                                  ...
                                                  shape[3],
                                                  dtype=torch.float32)
                altered_input_layer[:, :3, :, :] = encoder_ckpt[
            ...
            for p, v in encoder_ckpt.items():
                for original_name, psp_name in RESNET_MAPPING.items():
                    if original_name in p:
                        mapped_encoder_ckpt[p.replace(original_name,
                                                      psp_name)] = v

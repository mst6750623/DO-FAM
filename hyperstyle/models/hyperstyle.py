...
from .encoders.psp import pSp
from .stylegan2.model import Generator
from ..configs.paths_config import model_paths
from .hypernetworks.hypernetwork import SharedWeightsHyperNetResNet, SharedWeightsHyperNetResNetSeparable
...
class HyperStyle(nn.Module):
    ...
    def __init__(self, opts):
        ...
        self.decoder = Generator(self.opts.output_size,
                                 512,
                                 8,
    ...
    def set_hypernet(self):
        if self.opts.output_size == 1024:
            ...
            raise ValueError(
                f"Invalid Output Size! Support sizes: [1024, 512, 256]!")
        networks = {
            "SharedWeightsHyperNetResNet":
            SharedWeightsHyperNetResNet(opts=self.opts),
            "SharedWeightsHyperNetResNetSeparable":
            SharedWeightsHyperNetResNetSeparable(opts=self.opts),
        }
    ...
    def load_weights(self):
        if self.opts.checkpoint_path is not None:
            print(
                f'Loading HyperStyle from checkpoint: {self.opts.checkpoint_path}'
            )
            ckpt = torch.load(self.opts.checkpoint_path, map_location='cpu')
            self.hypernet.load_state_dict(self.__get_keys(ckpt, 'hypernet'),
                                          strict=True)
            self.decoder.load_state_dict(self.__get_keys(ckpt, 'decoder'),
                                         strict=True)
    ...
    def forward(self,
                ...
                return_weight_deltas_and_codes=False,
                weights_deltas=None,
                y_hat=None,
                codes=None):
        ...
        if input_code:
            ...
            if weights_deltas is None:
                ...
                weights_deltas = [
                    weights_deltas[i] + hypernet_outputs[i]
                    if weights_deltas[i] is not None else None
    ...
    def __get_hypernet_checkpoint(self):
        ...
        if self.opts.input_nc != 3:
            ...
            altered_input_layer = torch.randn(shape[0],
                                              self.opts.input_nc,
                                              shape[2],
                                              shape[3],
                                              dtype=torch.float32)
        ...
        for p, v in hypernet_ckpt.items():
            for original_name, net_name in RESNET_MAPPING.items():
                if original_name in p:
                    mapped_hypernet_ckpt[p.replace(original_name,
                                                   net_name)] = v

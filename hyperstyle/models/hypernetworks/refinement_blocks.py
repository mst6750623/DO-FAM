...
sys.path.append('.')
sys.path.append('..')
from ..encoders.helpers import SeparableBlock
...
class RefinementBlock(Module):
    ...
    def __init__(self,
        ...
        self.opts = opts
        self.kernel_size, self.in_channels, self.out_channels = PARAMETERS[
            self.layer_idx]
    ...
    def forward(self, x):
        ...
        if self.layer_idx in TO_RGB_LAYERS:
            ...
            x = x.view(-1, self.out_channels, self.in_channels)
            x = x.unsqueeze(3).repeat(1, 1, 1,
                                      self.kernel_size).unsqueeze(4).repeat(
                                          1, 1, 1, 1, self.kernel_size)
        return x
...
class RefinementBlockSeparable(Module):
    ...
    def __init__(self,
        ...
        if self.layer_idx in TO_RGB_LAYERS:
            ...
            self.output = Sequential(
                SeparableBlock(input_size=self.out_c,

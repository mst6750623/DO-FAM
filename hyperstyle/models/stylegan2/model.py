...
from .op.my_upfirdn2d import upfirdn2d
from .op.my_fused_act import FusedLeakyReLU, fused_leaky_relu
...
class ModulatedConv2d(nn.Module):
    ...
    def forward(self, input, style, weights_delta=None):
        ...
        if weights_delta is None:
            ...
            #print('weight device:',weights_delta.device)
            #weights_delta = torch.from_numpy(weights_delta).to(self.weight.device)
            #print('weights_delta type:', type(weights_delta))
            if type(weights_delta) is np.ndarray:
                weights_delta = torch.from_numpy(weights_delta).to(
                    self.weight.device)
...
class Generator(nn.Module):
    ...
    def forward(self,
        ...
        for style in styles:
            if not style.is_cuda and input_is_latent == True:
                raise TypeError('Not a cuda tensor')
        total_convs = len(self.convs) + len(
            self.to_rgbs) + 2  # +2 for first conv and toRGB
        ...
        out = self.conv1(out,
                         latent[:, 0],
                         noise=noise[0],
                         weights_delta=weights_deltas[0])
        ...
        for conv1, conv2, noise1, noise2, to_rgb in zip(
            ...
            out = conv1(out,
                        latent[:, i],
                        noise=noise1,
                        weights_delta=weights_deltas[weight_idx])
            out = conv2(out,
                        latent[:, i + 1],
                        noise=noise2,
                        weights_delta=weights_deltas[weight_idx + 1])
            skip = to_rgb(out,
                          latent[:, i + 2],
                          skip,

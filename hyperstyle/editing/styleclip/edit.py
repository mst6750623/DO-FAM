...
def parse_args():
    ...
    parser.add_argument(
        "--exp_dir",
        type=str,
        default="/mnt/pami23/yfyuan/EXP/TEST_hyperstyle_rec_1024",
        help=
        "Path to inference results with `latents.npy` saved here (obtained with inference.py)."
    )
    parser.add_argument(
        "--weight_deltas_path",
        type=str,
        ...
        help=
        "Root path holding all weight deltas (obtained by running inference.py)."
    )
    parser.add_argument(
        '--n_images',
        type=int,
    ...
    parser.add_argument("--beta_max", type=float, default=0.12)
    parser.add_argument("--num_betas", type=int, default=1)
    parser.add_argument("--alpha_min", type=float, default=5)
    parser.add_argument("--alpha_max", type=float, default=6)
    parser.add_argument("--num_alphas", type=int, default=1)
    parser.add_argument("--delta_i_c",
                        type=str,
                        default=edit_paths["styleclip"]["delta_i_c"],
                        help="path to file containing delta_i_c")
    parser.add_argument("--s_statistics",
                        type=str,
                        default=edit_paths["styleclip"]["s_statistics"],
                        help="path to file containing s statistics")
    parser.add_argument("--text_prompt_templates",
                        default=edit_paths["styleclip"]["templates"])
...
def load_direction_calculator(args):
    ...
    global_direction_calculator = StyleCLIPGlobalDirection(
        delta_i_c, s_std, text_prompt_templates)
    return global_direction_calculator


def load_stylegan_generator(args):
    stylegan_model = Generator(args.stylegan_size,
                               512,
                               8,
                               channel_multiplier=2).cuda()
...
def run():
    ...
    latents = np.load(os.path.join(args.exp_dir, 'latents.npy'),
                      allow_pickle=True).item()
    # prepare output directory
    #args.output_path = os.path.join(args.exp_dir, "styleclip_edits", f"{args.neutral_text}_to_{args.target_text}")
    args.output_path = os.path.join(
        '/home/stma/workspace/lattrans_hyperstyle/l2m/inference/hyperstyle_edit/smile0.11&5',
        "styleclip_edits", f"{args.neutral_text}_to_{args.target_text}")
    os.makedirs(args.output_path, exist_ok=True)
    # edit all images
    for idx, (image_name, latent) in enumerate(tqdm(latents.items())):
        if args.n_images is not None and idx >= args.n_images:
            break
        edit_image(image_name, latent, stylegan_model,
                   global_direction_calculator, args)


def edit_image(image_name, latent, stylegan_model, global_direction_calculator,
               args):
    ...
    weight_deltas = np.load(os.path.join(args.weight_deltas_path,
                                         image_name.split(".")[0] + ".npy"),
                            allow_pickle=True)
    weight_deltas = [
        torch.from_numpy(w).cuda() if w is not None else None
        for w in weight_deltas
    ]

    with torch.no_grad():

        source_im, _, latent_code_s = stylegan_model(
            [latent_code_i],
    ...
    results = []
    for beta in betas:
        direction = global_direction_calculator.get_delta_s(
            args.neutral_text, args.target_text, beta)
        edited_latent_code_s = [[
            s_i + alpha * b_i for s_i, b_i in zip(latent_code_s, direction)
        ] for alpha in alphas]
        edited_latent_code_s = [
            torch.cat(
                [edited_latent_code_s[i][j] for i in range(args.num_alphas)])
            for j in range(len(edited_latent_code_s[0]))
        ]
        for b in range(0, edited_latent_code_s[0].shape[0]):
            edited_latent_code_s_batch = [
                s_i[b:b + 1] for s_i in edited_latent_code_s
            ]
            with torch.no_grad():
                edited_image, _, _ = stylegan_model(
                    [edited_latent_code_s_batch],
    ...
    torchvision.utils.save_image(
        results,
        f"{args.output_path}/{image_name.split('.')[0]}.jpg",
        normalize=True,
        range=(-1, 1),
        padding=0,
        nrow=args.num_alphas)
